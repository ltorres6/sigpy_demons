...
def demonsUpdate(
    ...
    """
    if variant == "passive":
        V, loss = passiveUpdate(
            Is, Im, T, alpha, dS, dSMag, scalingFactor, device=device
        )
        return V, loss
    elif variant == "active":
        ...
        V, loss = inverseConsistentUpdate(
            Is, Im, T, alpha, D, scalingFactor, device=device
        )
        if Tinv is None:
            Tinv = -1 * T
        # Swap Inputs and use inverse
        V2, loss2 = inverseConsistentUpdate(
            Im, Is, Tinv, alpha, D, scalingFactor, device=device
        )
        V = (
            V - V2
        ) * 0.5  # This might have to be done in the log domain to be correct...
        loss = (loss + loss2) * 0.5
        return V, loss
    else:
